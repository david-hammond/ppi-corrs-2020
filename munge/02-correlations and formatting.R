...
# sets  a list of spurious vars, and removes others iteratively from TOC using grepl()
...
all = readRDS("./data/all-correlations.rds") %>% 
  filter(signif == "***") %>% 
  select( -n, -p, -signif)
...
pillars = rio::import("./data/results-correlations-2018-internalpeace.csv")  %>% 
  select(variablename1, category) %>%
  distinct() %>% 
...
thedata = all %>% 
  ...
  top_n(3, abs(r)) %>% 
  ungroup() %>% 
  filter(!(category %in% c("environmental", "other", "statistical", "np")))
